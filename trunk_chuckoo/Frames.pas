...
procedure TFFrames.FormMouseWheelDown(Sender: TObject; Shift: TShiftState;
  MousePos: TPoint; var Handled: Boolean);
begin
//self.VertScrollBar.Position :=  self.VertScrollBar.Position + (framelist[0] AS TCutFrame).Image.Height;
//self.VertScrollBar.Position :=  self.VertScrollBar.Position +  round((Settings.FramesHeight + round(Screen.PixelsPerInch / 4) -20 ) /2);
self.VertScrollBar.Position :=  self.VertScrollBar.Position +  round((Settings.FramesHeight  ) /1.49);
//self.VertScrollBar.Position :=  self.VertScrollBar.Position - ((framelist[1] AS TCutFrame).BStart.Top) + ((framelist[0] AS TCutFrame).BStart.Top);


end;

procedure TFFrames.FormMouseWheelUp(Sender: TObject; Shift: TShiftState;
  MousePos: TPoint; var Handled: Boolean);
begin
//self.VertScrollBar.Position :=  self.VertScrollBar.Position - (framelist[0] AS TCutFrame).Image.Height;
//self.VertScrollBar.Position :=  self.VertScrollBar.Position - round((Settings.FramesHeight + round(Screen.PixelsPerInch / 4) -20 ) /2);
